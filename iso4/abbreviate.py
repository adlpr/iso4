...
def abbreviate(title, periods=True, disambiguation_langs=set()):
    ...
    Inputs:
        ...
        (iterable) disambiguation_langs
            ...
            In cases where a disambiguating language is required but not
                supplied, function will raise an Exception.
            Default empty set.
    Output:
    ...
    for iw, orig_word in enumerate(title_words):
        ...
        word_candidates = (
            (word_norm, word_lemma) if word_norm != word_lemma else (word_norm,)
        )
        ...
        for word in word_candidates:
            ...
            if FULLWORD in CONFLICT_MAP and word in CONFLICT_MAP[FULLWORD]:
                ...
                if len(possible_langs) == 1:
                    ...
                    raise Exception(
                        "Ambiguous word in title: {}; must disambiguate between langs: {}".format(
...
def __initialize_ltwa():
    ...
    json_filepath = os.path.join(
        os.path.dirname(__file__), "ltwa_{}.json".format(LTWA_VERSION)
    )
    try:
        # Read JSON.
        ...
        csv_filepath = os.path.join(
            os.path.dirname(__file__), "ltwa_{}.csv".format(LTWA_VERSION)
        )
    ...
    with open(sw_filepath, "r") as inf:
        STOPWORDS = set(
            [unicodedata.normalize("NFKD", line.strip()) for line in inf.readlines()]
        )
    # Set stopwords keep as last from txt
    swkal_filepath = os.path.join(
        os.path.dirname(__file__), "stopwords_keep_as_last.txt"
    )
    ...
    TOKENIZER_REGEX = re.compile(
        "({}|\\s+)".format(
            "|".join(["(?:^|\\s){}(?:\\s|$)".format(w) for w in MULTI_WORD_TERMS])
        ),
        flags=re.I,
    )
...
def __normalize_abbr(abbr):
    ...
    for part in abbr.split(" "):
        parts.append(
            unicodedata.normalize("NFKD", part.strip("- ").rstrip(".").lower())
        )
    return " ".join(parts)
